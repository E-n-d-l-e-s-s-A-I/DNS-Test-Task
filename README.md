...
- **GET /sales**
  ...
  - Параметры: 
    - `city_id: int` - ID города, где совершены продажи
    - `store_id: int` - ID магазина, где совершены продажи
    - `product_id: int` - ID товара, который должен присутствовать в продажах
    - `days: int` - Максимальное количество дней прошедшее с момента продаж, включительно
  ...
  - Ответ: Возвращает массив продаж. Продажа состоит из: ID продажи, ID магазина, массив товаров продажи, общей суммы и общего количества товаров.
	```
    [
      {
        "id": 1,
	...
	```
  - Ошибки:
    - 422 Unprocessable Entity: Если не удалось привести параметры звпроса к указанным типам. Например если city_id это строка.
...
- **Get /sales/{sale_id}**
  ...
  - Ответ: Возвращает ID продажи, ID магазина, массив товаров продажи, обющую сумму и общее количество товаров
  ```
  {
    "id": 1,
    "store_id": 1,
...
- **Get /sales/{sale_id}/products**
  - Описание: Получение продажи и ее списка товаров продажи по ID. В отличие от **`Get /sales/{sale_id}`** возвращает детальную информации о каждом товаре, включая его название
...
- **Patch /sales/{sale_id}/products/{product_id}**
  ...
  ```
  - Ответ: Возвращает ID товара, количество товара, и его цену на момент продажи
  ```
  {
    "product_id": 1,
    "quantity": 2
