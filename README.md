...
- ## Среда-разработки
    ...
    ```
    #### 4. Теперь приложение запущено на `http://127.0.0.1:5000/`
    Ознакомится с конечным точками API можно по адресу `http://127.0.0.1:5000/docs`
...
# Структура проекта
    ...
    │   ├── .env-dev                          # Среда разработки для docker-compose
    |   └── .env-prod                         # Среда разработки для docker-compose
    │
    ...
    ├── docker-compose.yaml                 # Docker-compose для продакшен среды
...
Сложный параметрический запрос к базе данных был протестирован путем сравнения его результатов с фильтрацией на Python.
