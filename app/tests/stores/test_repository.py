...
async def test_get_objects(session, stores):
    stores_models = await StoreRepository.get_objects(session=session)
...
async def test_get_object(session, stores):
    store_model = await StoreRepository.get_object(
...
async def test_update_partial_object(session, store_add_data):
    store_model: Store = await StoreRepository.update_partial_object(
        session=session,
        object_id=1,
        data=store_add_data,
    )
    assert store_add_data.name == store_model.name
...
async def test_delete_object(session, stores):
    ...
    store_model = await StoreRepository.delete_object(
        session=session,
        object_id=store_id,
        )
    store_from_rep = store_model.to_dict()
